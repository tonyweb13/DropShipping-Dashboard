...
class Index extends Component {
    ...
    componentDidMount() {
        ...
        if (state) {
            ...
            this.setState({
                company: AppState.store,
                companydisable: disablecompany,
            });
        }
    }
    getUserAccountInfo = (data) => {
        axios.get("api/profile").then((response) => {
            ...
            let cname =
                userinfo.company_name != "" && !isEmpty(userinfo.company_name)
                    ? userinfo.company_name
                    : this.state.company;
            this.setState({ companyname: cname, emailaddress: userinfo.email });
        });
    };
    handleProfileSubmit(e) {
        ...
        if (haserrormsg == false) {
            axios.post("api/updateuserprofile", userData).then((response) => {
                this.setState({ formSubmitting: false });
                if (response.data.errormsg == "oldpasserror") {
                    ...
                    if (response.data.success == "EmailTaken") {
                        this.setState({
                            etakenemail: "Email is already taken.",
                        });
                    } else {
                ...
                <Box component={"form"}>
                    ...
                    <DialogContent>
                        <Typography component="div" className="dlg_msg">
                            ...
                            {this.state.reloginmsg != "" ? (
                                <div>{this.state.reloginmsg}</div>
                            ) : (
                                ""
                            )}
                        </Typography>

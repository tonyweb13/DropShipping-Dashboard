...
class Orders extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({ intransit: FilterAppState.intransit });

        this.getData({
            page: 1,
            perpage: this.state.perpage,
            datefilter: FilterAppState.intransit,
        });
        this.getChartData({ datefilter: FilterAppState.intransit });
    ...
    getData = (data) => {
        ...
        // axios.get('api/ordersintransit?page=' + data.page + '&perpage=' + this.perpageno + '&statusfilter=' + this.statfilter+ '&sortfield=' + this.sortname+ '&sorting=' + this.sortascdesc).then(response => {
        //     this.setState({orders:response.data.orders.data, data:response.data.orders});
        // });
        axios
            .get(
                "api/ordersintransit?statusfilter=" +
                    this.statfilter +
                    "&datefilter=" +
                    this.datefilter +
                    "&start_date=" +
            ...
            .then((response) => {
                ...
                response.data.orders.forEach(function (key) {
                    let row_grid_data = {
                        ...
                        shipping_date: key.ShipDate,
                        shipping_country: key.ShipCountry,
                        carrier: key.Carrier,
                        service_used: key.service_used,
                        delivery_status: key.DeliveryStatus,
    ...
    getChartData = (data) => {
        ...
        axios
            .get(
                "api/intransitchartdata?datefilter=" +
                    this.datefilter +
                    "&start_date=" +
                    this.start_date +
                    "&end_date=" +
                ...
                <Box component={"form"} onSubmit={this.handleInventorySubmit}>
                    ...
                    <DialogContent>
                        ...
                        <CSVLink
                            ...
                            filename={"In Transit Order Data"}
                        ...
                        <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
                            <Grid container spacing={3}>
                                <Grid item xs={12}>
                                    ...
                                    >
                                        Here is the graph for the in-transit
                                        orders.
                                    </Typography>
                                    <Typography
                                        component="div"
                              ...
                              <TableBody>
                                {this.state.orders.map((row) => (
                                  <TableRow key={row.id}> 
                                    ...
                                    <TableCell className={'st'+row.delivery_status.replace(/ /g, '') +' order-status-green'}>{row.delivery_status}</TableCell>
